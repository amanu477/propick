...
export function ProductCard({ product, rank }: ProductCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="p-6 md:p-8 grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
        ...
        <div className="md:col-span-3 flex flex-col items-center justify-center text-center space-y-4">
          <div className="w-full aspect-[3/2] bg-gray-50 rounded-xl flex items-center justify-center p-4 relative overflow-hidden">
            {product.badge && (
              <Badge variant="secondary" className="absolute top-2 right-2 text-[10px] bg-white shadow-sm">
                {product.badge}
              </Badge>
            )}
            <img 
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-center gap-1">
              {[1, 2, 3, 4, 5].map((s) => (
                <Star 
                  key={s} 
                  className={`w-4 h-4 ${s <= Number(product.rating) ? 'text-warning fill-warning' : 'text-gray-200'}`} 
                />
              ))}
            </div>
            <p className="text-sm font-medium text-gray-500">{avgScore}/10 Overall Score</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 pt-2">
            <div className="space-y-1">
              <div className="flex justify-between text-xs font-medium text-gray-500">
                <span>Speed</span>
                <span>{product.scores.speed}</span>
              </div>
              <div className="h-1.5 w-full bg-gray-100 rounded-full overflow-hidden">
                <div className="h-full bg-blue-500 rounded-full" style={{ width: `${product.scores.speed * 10}%` }} />
              </div>
            </div>
            <div className="space-y-1">
              <div className="flex justify-between text-xs font-medium text-gray-500">
                <span>Security</span>
                <span>{product.scores.security}</span>
              </div>
              <div className="h-1.5 w-full bg-gray-100 rounded-full overflow-hidden">
                <div className="h-full bg-indigo-500 rounded-full" style={{ width: `${product.scores.security * 10}%` }} />
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Pros & Cons Expansion (Optional/Inline) */}
